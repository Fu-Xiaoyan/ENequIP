...
import ase
# copied from https://github.com/SINGROUP/cluskit.git

def mic(D, cell):
    Dmic, D_len = find_mic(D.reshape(-1, 3), cell)
    Dmic = Dmic.reshape(D.shape)
    return Dmic
...
def micFaceCenter(coords, cell):
    ...
    """
    d = numpy.zeros((3))
    d[0] = numpy.linalg.norm(mic(coords[1] - coords[2], cell))
    d[1] = numpy.linalg.norm(mic(coords[2] - coords[0], cell))
    d[2] = numpy.linalg.norm(mic(coords[0] - coords[1], cell))
    p = d[0] + d[1] + d[2]
    d /= p
    c = numpy.zeros((3))

    micCoords = numpy.zeros((3, 3))
    micCoords[0] = coords[0]
    micCoords[1] = coords[0] + mic(coords[1] - coords[0], cell)
    micCoords[2] = coords[0] + mic(coords[2] - coords[0], cell)

    for k in range(3): c += micCoords[k] * d[k]
    return c


def micFaceNormal(coords, cell, normalise=True):
    """
    Computes the normal vector of a triangle.
    ...
    """
    r0 = coords[0]
    v1 = mic(coords[1] - r0, cell)
    v2 = mic(coords[2] - r0, cell)
    normal = numpy.cross(v1, v2)
    if normalise: normal /= numpy.linalg.norm(normal)
            ...
            for k in range(j + 1, natm):
                ...
                for l in range(k + 1, natm):
                    ...
                    for f in range(4):  # loop through the tris faces
                        ...
                        if not found:
                            tris = tris + [
                                [tri, triNormal, triCenter, newtri, triNormal / numpy.linalg.norm(triNormal)]]
...
def periodic_delaunator(xyz, cell, rcut):
    """Delaunay algorithm which compartmentalizes a volume in
    real space comprised of a set of points. The points
    become vertices of tetrahedra which do not overlap.
    As a side product, non-overlapping triangles are defined
    ...
    # minimum image convention vector and distance matrices
    # TODO: not used in code yet! replace all calls of the mic function
    dvec, dmat = ase.geometry.get_distances(p1=xyzShake,
                                            p2=xyzShake,
                                            cell=cell, pbc=True)
    ...
    for i in range(natm):
        for j in range(i + 1, natm):
            d = mic(xyz[i] - xyz[j], cell);
            d = numpy.dot(d, d)
            if d > r2: continue

            for k in range(j + 1, natm):
                d = mic(xyz[i] - xyz[k], cell);
                d = numpy.dot(d, d)
                if d > r2: continue
                d = mic(xyz[j] - xyz[k], cell);
                d = numpy.dot(d, d)
                if d > r2: continue

                for l in range(k + 1, natm):
                    d = mic(xyz[i] - xyz[l], cell);
                    d = numpy.dot(d, d);
                    if d > r2: continue
                    d = mic(xyz[j] - xyz[l], cell);
                    d = numpy.dot(d, d);
                    if d > r2: continue
                    d = mic(xyz[k] - xyz[l], cell);
                    d = numpy.dot(d, d);
                    if d > r2: continue
                    ...
                    for f in range(4):
                        coords = xyz[tfaces[f]]
                        Ai[f] = 0.5 * numpy.linalg.norm(
                            numpy.cross(mic(coords[1] - coords[0], cell), mic(coords[2] - coords[0], cell)))

                    # compute the incenter of the tetrahedron
                    incenter = numpy.zeros((3))
                    micTetraCoords = numpy.zeros((4, 3))
                    for cc in range(4):
                        if cc == 0:
                            micTetraCoords[cc] = tetraCoords[cc]
                        else:
                            micTetraCoords[cc] = tetraCoords[0] + mic(tetraCoords[cc] - tetraCoords[0], cell)
                    for cc in range(4):
                        incenter = incenter + Ai[cc] * micTetraCoords[cc]
                    incenter /= numpy.sum(Ai)
                    incenters = incenters + [incenter]
                    ...
                    for f in range(4):  # loop through the tris faces
                        ...
                        # compute normal
                        triNormal = micFaceNormal(coords, cell, False)
                        triCenter = micFaceCenter(coords, cell)

                        # tetraNormals[f] = triCenter - incenter

                        if numpy.dot(mic(triCenter - incenter, cell), triNormal) < 0:
                            newtri = numpy.flip(tri, axis=0)
                            triNormal *= -1
                        else:
                            newtri = tri
    ...
    for i in range(edges.shape[0]):
        ...
        edgeInfo[i, 0:3] = xyz[e[0]] + 0.5 * mic(xyz[e[1]] - xyz[e[0]], cell)
        edgeInfo[i, 3:] = enrm
...
def get_surface_atoms(atoms, mask=False):
    """Determines the surface atoms of the slab using delaunay
    triangulation.
    ...
    ATTENTION: Do not use too small cells in x and y as the Delaunay
    algorithm may fail!
    The attributes
    ...
    if atoms.has('surface'):
        if mask:
            ...
            surface_mask[atoms.arrays['surface']] = True
            return surface_mask
        else:
            return atoms.arrays['surface']

    pos = atoms.get_positions()
    # delaunator not only gets surface atoms but also adsorption site locations

    summary_dict = periodic_delaunator(pos, atoms.cell, atoms.max_bondlength)

    ids_surface_atoms = summary_dict["ids_surface_atoms"]

    # store in attributes
    atoms.arrays['surface'] = ids_surface_atoms
...
def main():
    ...
    import ase.io
    atoms = ase.io.read('test_surface_identify/Cu_100_slab.xsd')

    atoms.max_bondlength = 2.5
    surface_mask = get_surface_atoms(atoms, mask=True)

    c = FixAtoms(mask=surface_mask)
